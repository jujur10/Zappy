...
public:
    ...
    AtomicQueueB(AtomicQueueB&& b) noexcept
        : AllocatorElements(static_cast<AllocatorElements&&>(b)) // : This must be noexcept, static_assert that.
    ...
    ~AtomicQueueB() noexcept {
        if(elements_) {
            ...
            AllocatorElements::deallocate(elements_, size_); // : This must be noexcept, static_assert that.
        }
    }
...
class AtomicQueueB2 : private std::allocator_traits<A>::template rebind_alloc<unsigned char>,
    ...
    void deallocate_(U* p) noexcept {
        StorageAllocator::deallocate(reinterpret_cast<unsigned char*>(p), size_ * sizeof(U)); // : This must be noexcept, static_assert that.
    }
...
public:
    ...
    AtomicQueueB2(AtomicQueueB2&& b) noexcept
        : StorageAllocator(static_cast<StorageAllocator&&>(b)) // : This must be noexcept, static_assert that.
        , Base(static_cast<Base&&>(b))
        , size_(std::exchange(b.size_, 0))

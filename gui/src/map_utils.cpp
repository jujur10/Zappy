...
Vector2 GetCoordsFromTileMatrix(const raylib::Matrix &transformMatrix)
{
    ...
    uint32_t tileX = static_cast<uint32_t>(std::round(x));
    uint32_t tileY = static_cast<uint32_t>(std::round(y));
...
{
    return static_cast<uint64_t>(coords.y) * static_cast<uint64_t>(kMAP_WIDTH) + static_cast<uint64_t>(coords.x) + 1 + FLECS_HI_ID_RECORD_ID;
}
...
{
    Vector2 coords = GetCoordsFromTileMatrix(transformMatrix);
